...
export default async function Page(props: {
  ...
  const matched = query ? await prisma.certification.count({ where }) : total;
  ...
  const positions = query
    ? Object.fromEntries(
        (await prisma.certification.findMany({ select: { id: true }, orderBy: ORDER_BY })).map(
          (r, i) => [r.id, i]
        )
      )
    : undefined;
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          {query
            ? `${matched} of ${total} certification${total !== 1 ? "s" : ""} match “${query}” · clear the search to reorder`
            : `${total} certification${total !== 1 ? "s" : ""} total · drag to reorder within the page, or type a position to move one anywhere`}
        </p>
      </div>

      <ListSearch defaultValue={query} placeholder="Search certifications by name or alt text..." />

      <CertificationList certifications={rows} inUse={inUse} offset={offset} total={total} positions={positions} query={query} />
      <Pagination page={page} totalPages={totalPages} total={matched} perPage={PER_PAGE} extraParams={query ? `q=${encodeURIComponent(query)}` : ""} basePath="/certifications" />
    </div>
  );
}
